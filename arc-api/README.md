...
| Datapacks | `dev.arc.api.datapack.datapackMaker` | validated 1.21.4 biome, loot, tag, function, recipe, advancement, and predicate generation |
| PDC schemas | `dev.arc.api.pdc.pdcSchema` | reusable typed fields, defaults, validation, and mutation for any `PersistentDataHolder` |
| Player PDC schema | `dev.arc.api.player.playerDataSchema` | typed defaults, validation, lazy legacy-key migration, and mutation operators |
...
Generate and deploy data-driven content without hand-building directory trees
or interpolating JSON:

```kotlin
plugin.datapackMaker("arc_content") {
    lootTable("arc:chests/dungeon") {
        pool(rolls = 1..3) {
            entry("minecraft:diamond", weight = 1, count = 1..2)
            empty(weight = 4)
        }
    }
    tag("blocks", "arc:valuable_ores") {
        add("minecraft:diamond_ore")
        addTag("minecraft:gold_ores")
    }
    function("arc:load") {
        +"say Arc content loaded"
    }
}.deployAndReload()
```

Legacy plural tag names such as `blocks` and `items` are normalized to the
Minecraft 1.21.4 singular directories. Resource IDs and output paths are
validated, JSON is parsed and re-emitted safely, and an Arc manifest removes
only stale files generated by previous deployments.

## PDC schemas

Use one immutable schema across players, entities, chunks, worlds, and item
metadata instead of repeating a `PersistentDataType` token at every call:

```kotlin
lateinit var kills: PdcField<Int, Int>

val data = pdcSchema("myplugin") {
    kills = int("kills") {
        default(0)
        validate("kills must be non-negative") { it >= 0 }
    }
...
Use `plugin.pdcSchema { ... }` to derive the namespace from a plugin. Custom
`PersistentDataType` implementations are supported through `custom(...)`.
For player fields that require lazy migration from old keys, use the specialized
`playerDataSchema` API below.

## Player PDC schema

Declare persistent player fields once and keep validation and migrations next
to the field definition:

```kotlin
lateinit var coins: PlayerStore<Int, Int>

val playerData = plugin.playerDataSchema {
    coins = int("coins") {
        default { 0 }
...
player[coins] = 10
player.increment(coins, 5)
val current = player.getOrDefault(coins)
```

The current key always wins. When it is absent, reads move the first matching
legacy value to the current key after validation. All setters, defaults, and
mutation results pass through the same validator.
